    ...
    def op_gate(self, state):
        if is_non_idle_and_non_empty(
            ...
            if ANALYSIS_MODULE_LOADED:  # 🔍
                record_layer_value("router_inputs", state.hidden_states_mlp_input, self.layer_id)
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]], is_nextn=False):
        ...
        if ANALYSIS_MODULE_LOADED:  # 🔍
            if not analysis_utils.ANALYSIS_ENABLED:
                ...
                for layer_id, decoder in enumerate(self.model.layers):
                    if "router_weights" in ANALYSIS_TYPE:
                        if isinstance(decoder.mlp, DeepseekV2MoE):
                            record_layer_weights("router_weights", decoder.mlp.gate.weight.data, layer_id)
                    ...
                    if "router_bias" in ANALYSIS_TYPE:
                        if isinstance(decoder.mlp, DeepseekV2MoE) and isinstance(decoder.mlp.gate.e_score_correction_bias, nn.Parameter):
                            record_layer_weights("router_bias", decoder.mlp.gate.e_score_correction_bias.data, layer_id)

                    if "layernorm_weights" in ANALYSIS_TYPE:
                        record_layer_weights("pre_attn_norm_weight", decoder.input_layernorm.weight.data, layer_id)
                        record_layer_weights("pre_mlp_norm_weight", decoder.post_attention_layernorm.weight.data, layer_id)

                    for param_name, param in decoder.named_parameters():
                        record_layer_weights_magnitude(param_name, param.data, layer_id)
